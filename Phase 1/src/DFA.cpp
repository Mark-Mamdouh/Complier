...
void DFA::fill_subset_construction(int start_node_num, vector<State> all_nfa_nodes)
{
    ...
    for (int dfa_index =0 ; dfa_index < dfa_mapping.size() ; dfa_index++)// el size lma yt3'yar :/
    {
       ...
       cout << "DFA>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
        // Print subset Construction
       /* for(int state_num=0 ; state_num< dfa_mapping.size(); state_num++)
        {
            std::string str(dfa_mapping[state_num]->acceptance);
            cout << "\n " <<state_num <<"  accp   ((((" << str << ")))): ";
            for (set<int>::iterator j = dfa_mapping[state_num]->combined_states->begin(); j != dfa_mapping[state_num]->combined_states->end(); j++)
            {
                int pp =*j;
                cout << nfa_nodes[pp].num <<" " ;
            }
            cout << "\n " ;
            for(int n=52 ; n<INPUT.length() ; n++)
            {
                cout << subset_construction[state_num][n] << " ";
            }
            cout<< "\n ======\n" << dfa_state_num;
