...
void Matching ::match_input()
{
    ...
    derivatins_steps.push_back(std::make_pair(elements[0].st,emp));
        ...
        else if (!(LHS.terminal))  // not terminal
        {
            ...
            if(parsing_table.find(index_table)!= parsing_table.end())
            {
                ...
                else
                {
                    ...
                    vector<int> result_stack = refresh_stack();
                    derivatins_steps.push_back(std::make_pair(LHS.st,result_stack));
                }
            }
            else
            {
